...
with a request body for the specified collection. The rules for "_id" creation operate the same way on multiple documents as they do with a single
document.
...
The batch addition of documents only supports the default response.
...
.. code-block:: plaintext
        ...
        $ curl -v -X POST -H "Content-Type:application/json"  -H "X-Tapis-Token:$jwt" -d '{"docName":"test doc"}' $BASE_URL/v3/meta/
